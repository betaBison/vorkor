...
class visualization(QtCore.QThread):
    def __init__(self,type,num_intruders):
        ...
        itr_3d = np.empty((self.num_intruders),dtype=object)
        itr_pts = np.empty((flag.N,3,self.num_intruders)) 
        for k in range(self.num_intruders):
            ...
            itr_3d[k] = gl.GLMeshItem(meshdata=sphere_object, smooth=False, drawFaces=True, drawEdges=False, color=(0,1.0-float(k)/self.num_intruders,float(k)/self.num_intruders,1) )
            w.addItem(itr_3d[k])
