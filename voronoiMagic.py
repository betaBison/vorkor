...
class VoronoiMagic():
    ...
    def graph(self,step):
        self.E = []
        if step > 5:
            points = np.transpose(self.intruder_states[0:2,step,:])
            vor = Voronoi(points)
            ...
            for pointidx, simplex in zip(vor.ridge_points, vor.ridge_vertices):
                ...
                if np.any(simplex < 0):
                    ...
                    self.E.append([[vor.vertices[i,0], vor.vertices[i,1]],[far_point[0], far_point[1]]])
            ...
            for ii in range(self.E.shape[0]):
                plt.plot([self.E[ii,0,0],self.E[ii,1,0]],[self.E[ii,0,1],self.E[ii,1,1]],'m--')
